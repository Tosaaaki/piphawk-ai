...
class BaseScaleTest(unittest.TestCase):
    def setUp(self):
        self._added = []
        def add(name, mod):
    ...
    def tearDown(self):
        ...
        for key in ['SCALE_LOT_SIZE', 'AI_PROFIT_TRIGGER_RATIO', 'PIP_SIZE', 'SCALE_MAX_POS', 'SCALE_TRIGGER_ATR']:
            os.environ.pop(key, None)

class TestScaleConstraints(BaseScaleTest):
    def test_scale_limit(self):
        ...
        importlib.reload(jr)
        jr.instrument_is_tradeable = lambda instrument: True
        runner = jr.JobRunner(interval_seconds=1)
        runner._manage_pending_limits = lambda *a, **k: None
        runner.scale_count = 1
        with self.assertRaises(SystemExit):
            runner.run()
        self.assertEqual(len(runner.order_mgr.calls), 0)
