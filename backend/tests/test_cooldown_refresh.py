...
class TestRefreshAICooldowns(unittest.TestCase):
    def setUp(self):
        self._mods = []
        ...
        pm.get_account_balance = lambda *a, **k: None
        pm.get_open_positions = lambda *a, **k: []
        ...
        lm.log_policy_transition = lambda *a, **k: None
        ...
        comp.decide_trade_mode_detail = lambda *a, **k: ("trend_follow", None)
        add("signals.composite_mode", comp)

        rm = types.ModuleType("risk.portfolio_risk_manager")
        rm.PortfolioRiskManager = lambda *a, **k: None
        add("risk.portfolio_risk_manager", rm)

        sr = types.ModuleType("backend.strategy.risk_manager")
        sr.calc_lot_size = lambda *a, **k: 0
        add("backend.strategy.risk_manager", sr)

        prom = types.ModuleType("prometheus_client")
        prom.start_http_server = lambda *a, **k: None
        add("prometheus_client", prom)

        params = types.ModuleType("config.params_loader")
        params.load_params = lambda *a, **k: None
        params.save_last_mode = lambda *a, **k: None
        add("config.params_loader", params)

        self.runner_mod = importlib.import_module("backend.scheduler.job_runner")
        importlib.reload(self.runner_mod)
        os.environ["PIP_SIZE"] = "0.01"
        os.environ["OANDA_API_KEY"] = "x"
        os.environ["OANDA_ACCOUNT_ID"] = "x"
        os.environ["OPENAI_API_KEY"] = "x"
        os.environ["AI_COOLDOWN_SEC_OPEN"] = "30"
        os.environ["AI_COOLDOWN_SEC_FLAT"] = "60"
        self.runner = self.runner_mod.JobRunner(interval_seconds=1)
    ...
    def tearDown(self):
        ...
        for k in [
            ...
            "OANDA_ACCOUNT_ID",
            "OPENAI_API_KEY",
            "AI_COOLDOWN_SEC_OPEN",
            "AI_COOLDOWN_SEC_FLAT",
        ]:
            os.environ.pop(k, None)

    def test_refresh_ai_cooldowns(self):
        self.assertEqual(self.runner.ai_cooldown_open, 30)
        self.assertEqual(self.runner.ai_cooldown_flat, 60)
        os.environ["AI_COOLDOWN_SEC_OPEN"] = "10"
        os.environ["AI_COOLDOWN_SEC_FLAT"] = "20"
        self.runner.refresh_ai_cooldowns()
        self.assertEqual(self.runner.ai_cooldown_open, 10)
        self.assertEqual(self.runner.ai_cooldown_flat, 20)

    def test_reload_params_updates_cooldown(self):
        def load_params(path="config/strategy.yml", *a, **k):
            os.environ["AI_COOLDOWN_SEC_OPEN"] = "40"
            os.environ["AI_COOLDOWN_SEC_FLAT"] = "50"
        self.runner_mod.params_loader.load_params = load_params
        self.runner.reload_params_for_mode("scalp")
        self.assertEqual(self.runner.ai_cooldown_open, 40)
        self.assertEqual(self.runner.ai_cooldown_flat, 50)
