...
class TestReversalExit(unittest.TestCase):
    def setUp(self):
        self._mods = []
        def add(name, mod):
        ...
        os.environ["REVERSAL_EXIT_ATR_MULT"] = "0.5"
        os.environ["REVERSAL_EXIT_ADX_MIN"] = "20"
        ...
        oa.evaluate_exit = lambda *a, **k: types.SimpleNamespace(
            action="EXIT", reason="reversal", as_dict=lambda: {"action": "EXIT", "reason": "reversal"}
        )
    ...
    def test_exit_triggered_on_band_break(self):
